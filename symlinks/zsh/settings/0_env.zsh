export LC_CTYPE="en_US.UTF-8"

export EDITOR="nvim"
export VISUAL="nvim"

export GOPATH="$HOME/.gopath"
export GOBIN="$GOPATH/bin"
export HELMPATH="/usr/local/opt/helm@2/bin"
export HSPATH="$HOME/.cabal/bin"
export RSPATH="$HOME/.cargo/bin"
export RUBY_PATH="/usr/local/opt/ruby/bin"
export TEXBINDIR="/Library/TeX/texbin"
export GNUBIN="/usr/local/opt/gnu-getopt/bin:/usr/local/opt/coreutils/libexec/gnubin:/usr/local/opt/gnu-sed/libexec/gnubin:/usr/local/opt/findutils/libexec/gnubin"

export PATH="$GNUBIN:$HOME/.bin:$HOME/.local/bin:$HOME/.rbenv/shims:$RUBY_PATH:$HELMPATH:$PATH:$HSPATH:$RSPATH:$GOBIN:$TEXBINDIR:/usr/local/opt/gettext/bin"
